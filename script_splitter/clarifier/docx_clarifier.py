"""DOCX clarifier: uses python-docx to extract paragraph indentation for
layout-based text classification. Adapted from Prism's Docx_Clarifier."""
import re
import os
from .markdown_refiner import MarkdownRefine

def _scene_heading_pattern() -> re.Pattern:
    return re.compile(r"^(EXT\./INT\.|EXT\.|INT\.)\s+[A-Z0-9].*")

def docx_clarify(input_path: str, output_path: str | None = None) -> str:
    if not os.path.exists(input_path):
        raise FileNotFoundError(f"Input not found: {input_path}")
    from docx import Document
    doc = Document(input_path)
    cleaned_lines = []
    in_body = False
    heading_re = _scene_heading_pattern()

    for paragraph in doc.paragraphs:
        raw_text = paragraph.text
        text = raw_text.strip()
        if not text:
            continue

        left_indent = paragraph.paragraph_format.left_indent
        indent_inches = left_indent.inches if left_indent else 0.0
        leading_spaces = len(raw_text) - len(raw_text.lstrip())
        effective_indent = indent_inches + (leading_spaces * 0.1)

        if not in_body:
            if heading_re.match(text):
                in_body = True
            else:
                continue
        if "THE END" in text.upper():
            break
        if text.isdigit():
            continue
        text = re.sub(r"\s+", " ", text)

        char_test = re.sub(r"\s*\((?:CONT['’]?D|CONTINUED|V\.O\.|O\.S\.)\)", "", text, flags=re.IGNORECASE).strip()

        if heading_re.match(text):
            cleaned_lines.append(f"\n# {text}")
        elif char_test.isupper() and len(char_test) < 40 and not text.endswith((".", "?", "!")):
            cleaned_lines.append(f"\n### {char_test}")
        elif text.startswith("(") and text.endswith(")"):
            cleaned_lines.append(text)
        elif effective_indent > 1.0:
            cleaned_lines.append(f"> {text}")
        else:
            cleaned_lines.append(text)

    raw_md = "\n".join(cleaned_lines).strip()
    final_md = MarkdownRefine(raw_md)
    if output_path:
        os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
        with open(output_path, "w", encoding="utf-8") as f:
            f.write(final_md)
    return final_md

